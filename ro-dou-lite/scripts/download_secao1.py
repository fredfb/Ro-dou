#!/usr/bin/env python3
"""
Download and load INLABS DOU Section 1 data to SQLite.

This script specifically downloads and processes only Section 1 (DO1) files
from INLABS portal.

Usage:
    python download_secao1.py [--date YYYY-MM-DD] [--config path/to/config.yaml]

Examples:
    # Download today's Section 1 data
    python download_secao1.py

    # Download specific date
    python download_secao1.py --date 2025-01-15
"""
...
def filter_section1_files(all_files):
    """Filter only Section 1 files from downloaded list.

    Section 1 files typically have naming patterns like:
    - DO1_YYYY-MM-DD.zip
    - DO1E_YYYY-MM-DD.zip (extra edition)
    """
    section1_patterns = ['DO1', 'DO1E']
    filtered = []

    for file_path in all_files:
        file_name = Path(file_path).name.upper()
        if any(pattern in file_name for pattern in section1_patterns):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Download INLABS DOU Section 1 data'
    )
    parser.add_argument(
        '--date',
        type=str,
    ...
    try:
        ...
        # Filter Section 1 files only
        section1_urls = [url for url in all_file_urls
                         if 'DO1' in url.upper()]

        if not section1_urls:
            logger.warning(f"No Section 1 files found for {args.date}")
            return 0

        logger.info(f"Found {len(section1_urls)} Section 1 files (out of {len(all_file_urls)} total)")

        # Download Section 1 files
        downloaded_files = []
        date_dir = download_path / args.date
        date_dir.mkdir(parents=True, exist_ok=True)

        for file_url in section1_urls:
            try:
        ...
        for xml_file in xml_files:
            try:
                ...
                # Filter to ensure only Section 1 articles
                section1_articles = [
                    art for art in file_articles
                    if art.get('pubname', '').startswith('DO1')
                ]
                articles.extend(section1_articles)
            except Exception as e:
                logger.error(f"Failed to parse {xml_file.name}: {e}")
                continue

        if not articles:
            logger.warning("No Section 1 articles extracted from XML files")
            return 0

        # Load into database
        logger.info(f"Loading {len(articles)} Section 1 articles into database")
        ...
        # Print summary
        logger.info("=" * 60)
        logger.info(f"Section 1 Download Summary for {args.date}")
        logger.info(f"  Files found: {len(section1_urls)}")
        logger.info(f"  XML files processed: {len(xml_files)}")
        logger.info(f"  Section 1 articles parsed: {len(articles)}")
        ...
        logger.info(f"  Section 1 articles: {db.count_articles(sections=['DO1', 'DO1E'])}")
        logger.info("=" * 60)
