#!/usr/bin/env python3
"""
Download and load INLABS DOU Section 3 data to SQLite.

This script specifically downloads and processes only Section 3 (DO3) files
from INLABS portal.

Usage:
    python download_secao3.py [--date YYYY-MM-DD] [--config path/to/config.yaml]

Examples:
    # Download today's Section 3 data
    python download_secao3.py

    # Download specific date
    python download_secao3.py --date 2025-01-15
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Download INLABS DOU Section 3 data'
    )
    parser.add_argument(
        '--date',
        type=str,
    ...
    try:
        ...
        # Filter Section 3 files only
        section3_urls = [url for url in all_file_urls
                         if 'DO3' in url.upper()]

        if not section3_urls:
            logger.warning(f"No Section 3 files found for {args.date}")
            return 0

        logger.info(f"Found {len(section3_urls)} Section 3 files (out of {len(all_file_urls)} total)")

        # Download Section 3 files
        downloaded_files = []
        date_dir = download_path / args.date
        date_dir.mkdir(parents=True, exist_ok=True)

        for file_url in section3_urls:
            try:
        ...
        for xml_file in xml_files:
            try:
                ...
                # Filter to ensure only Section 3 articles
                section3_articles = [
                    art for art in file_articles
                    if art.get('pubname', '').startswith('DO3')
                ]
                articles.extend(section3_articles)
            except Exception as e:
                logger.error(f"Failed to parse {xml_file.name}: {e}")
                continue

        if not articles:
            logger.warning("No Section 3 articles extracted from XML files")
            return 0

        # Load into database
        logger.info(f"Loading {len(articles)} Section 3 articles into database")
        ...
        # Print summary
        logger.info("=" * 60)
        logger.info(f"Section 3 Download Summary for {args.date}")
        logger.info(f"  Files found: {len(section3_urls)}")
        logger.info(f"  XML files processed: {len(xml_files)}")
        logger.info(f"  Section 3 articles parsed: {len(articles)}")
        ...
        logger.info(f"  Section 3 articles: {db.count_articles(sections=['DO3', 'DO3E'])}")
        logger.info("=" * 60)
